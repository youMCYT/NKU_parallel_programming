...
double a[N], b[N][N], trivial_sum[N], optimized_sum[N];
...
int main(void)
{
    ...
    for (int j = 4; j <= N; j *= 2)
    {
        ...
        gettimeofday(&tail, NULL);
        cout << "Average time per optimized operation: " << ((tail.tv_sec - head.tv_sec) * 1000.0 + (tail.tv_usec - head.tv_usec) / 1000.0) / ::count << " ms" << endl;
    }
