        ...
        for (int k = 0; k < N; k++)
        {
            ...
            #pragma omp master
            {
                if (k % s == r)
                {
                    ...
                    if (k % s != (r + 1) % s)
                        MPI_Isend(&m3[k][k], N - k, MPI_FLOAT, (r + 1) % s, k, MPI_COMM_WORLD, &req);  // (r + 1) % s是环形意义上的下一个进程
                }
                else
                {
                    MPI_Recv(&m3[k][k], N - k, MPI_FLOAT, (r + s - 1) % s, k, MPI_COMM_WORLD, MPI_STATUS_IGNORE);  // (r + s - 1) % s是环形意义上的上一个进程
                    if (k % s != (r + 1) % s)
                        MPI_Isend(&m3[k][k], N - k, MPI_FLOAT, (r + 1) % s, k, MPI_COMM_WORLD, &req);  // 转发
                }
            }
