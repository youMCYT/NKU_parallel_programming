...
void gauss_mpi(int N)
{
    ...
    if (r == 0)
    {
        for (int i = 1; i < s; i++)
        {
            for (int j = get_min(N, s, i); j < get_max(N, s, i); j++)
                MPI_Send(&m3[j][0], N, MPI_FLOAT, i, j, MPI_COMM_WORLD);
        }
    }
    else
    {
        for (int i = min; i < max; i++)
            MPI_Recv(&m3[i][0], N, MPI_FLOAT, 0, i, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    ...
    #pragma omp parallel if(N > 512) num_threads(thread_num)
    {
        for (int k = 0; k < N; k++)
        {
            // int root = std::min((k - k % ((N - N % s) / s)) / ((N - N % s) / s), s - 1);
            ...
            #pragma omp master
            {
                if (k >= min && k < max)
                {
                    ...
                    if (r + 1 < s)
                        MPI_Isend(&m3[k][k], N - k, MPI_FLOAT, r + 1, k, MPI_COMM_WORLD, &req);  // 非阻塞式发送给下一个进程
                }
                else if (k < min)
                {
                    MPI_Recv(&m3[k][k], N - k, MPI_FLOAT, r - 1, k, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    if (r + 1 < s)
                        MPI_Isend(&m3[k][k], N - k, MPI_FLOAT, r + 1, k, MPI_COMM_WORLD, &req);  // 转发
                }
            }
    ...
    if (r == 0)
    {
        ...
        {
            for (int j = get_min(N, s, i); j < get_max(N, s, i); j++)
                MPI_Recv(&m3[j][0], N, MPI_FLOAT, i, j, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
    }
    else
    {
        for (int i = min; i < max; i++)
            MPI_Send(&m3[i][0], N, MPI_FLOAT, 0, i, MPI_COMM_WORLD);
    }
}
