...
// 动态线程实现的线程函数
void *threadFuncDynamic(void *param)
{
...
void gauss_pthread(int N)
{
    for (int k = 0; k < N; k++)
    {
        ...
        for (int t_id = 1; t_id < thread_num; t_id++)
            pthread_create(&handles[t_id], NULL, threadFuncDynamic, &param[t_id]);
...
int main(int argc, char *argv[])
{
    ...
    for (int n = 256; n <= size; n *= 2)
    {
        ...
        std::cout << "dynamic average time: " << elapsed3.count() / renum << " (ms)" << std::endl;
        elapsed1.zero();
        elapsed2.zero();
        elapsed3.zero();
    }
